...
if (dimension == 1) then
  ...
  do i = 2, nx+1
    do j = 2, ny+1
      ...
      b(i) = 0
      systemT(tracker, 1) = ap
      systemT(tracker-(Ny+2), 2) = -aw
      systemT(tracker+(Ny+2), 3) = -ae
      systemT(tracker + 1, 4) = -an
      systemT(tracker - 1, 5) = -as
      end do
  end do
end if

call jacobi(systemT, b, temperature, residual, itermax, dimension, nx, ny, nz)

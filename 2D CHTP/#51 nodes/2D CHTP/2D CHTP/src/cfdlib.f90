...
subroutine start (T,Tref)

	implicit none
	integer:: i,j
	real(KIND = DP)  :: Tref
	real(KIND = DP), dimension(Nx,Ny,itermax) :: T
...
!					NORTH					!
	Do i=1,(Nx)
		...
		an(i,Ny)=0.0D+00
		as(i,Ny)=0.0D+00
		if (i==1 .or. i==5) b(i,Ny)=0.0D+00
		if (i/=1 .and. i/=5) b(i,Ny)=100.0D+00
	end do
...
subroutine residual(T,ap,ae,aw,an,as,b,Res_T,Rmax_T)
	...
	do  j=2,Ny-1
		do i=2,Nx-1
			...
			Resi=dabs(ap(i,j)*T(i,j,iter)-ae(i,j)*T(i+1,j,iter)-aw(i,j)*T(i-1,j,iter)-as(i,j)*T(i,j-1,iter)-an(i,j)*T(i,j+1,iter)-b(i,j))
			summ=summ+(Resi*Resi)

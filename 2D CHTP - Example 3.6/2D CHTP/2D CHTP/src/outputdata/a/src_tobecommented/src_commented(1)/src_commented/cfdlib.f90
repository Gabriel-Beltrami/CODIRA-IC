...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE dependent_var
!>    @name      dependent_var
!!    @brief     Computes all the variables that depend on the user defined variables.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Stress LT
!>    @brief     Compute the stresses for the momentum transport equation, both the laminar and the turbulent stresses.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!				2.0D+0 * muT(i,Ny) * (V(i,Ny-1)-V(i,Ny-2)) / DYP(Ny-1)		!Fluid Boundary
	...
	Do i=1,(Nx-1)
		! South boundary
!		MUn= (muT(i,1)+muT(i+1,1)) /2.0D+0 						!Fluid Boundary
!		TxyT(i,1)=MUn * ((U(i,2)-U(i,1)) / DYV(1) + (V(i+1,1)-V(i,1)) / DXU(i))		!Fluid Boundary
		! North boundary
!		MUn= (muT(i,Ny)+muT(i+1,Ny)) /2.0D+0 							!Fluid Boundary
!		TxyT(i,Ny-1)=MUn * ((U(i,Ny)-U(i,Ny-1)) / DYV(Ny-1) + (V(i+1,Ny-1)-V(i,Ny-1)) / DXU(i)) !Fluid Boundary
	...
	Do j=2,(Ny-2)
		! West Boundary
!		MUn= (muT(1,j)+muT(1,j+1)) /2.0D+0 						!Fluid Boundary
!		TxyL(1,j)=MUn * ((U(1,j+1)-U(1,j)) / DYV(j) + (V(2,j)-V(1,j)) / DXU(1))		!Fluid Boundary
		! East Boundary
!		MUn= (muT(Nx,j)+muT(Nx,j+1)) /2.0D+0 						        !Fluid Boundary
!		TxyL(Nx-1,j)=MUn * ((U(Nx-1,j+1)-U(Nx-1,j)) / DYV(j) + (V(Nx,j)-V(Nx-1,j)) / DXU(Nx-1))	!Fluid Boundary
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE P Update
!>    @brief     Subroutine to copy the values of a matrix V into a matrix Vx, both of size Nx * Ny.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!	 Calculo da gamma
	...
	if (i==2) 	GIW = GIWL    ! Add turbulent viscosity if fluid boundary
	if (i==Nx-1) 	GIE = GIEL    ! Add turbulent viscosity if fluid boundary
	if (j==2) 	GIS = GISL    ! Add turbulent viscosity if fluid boundary
	if (j==Ny-1) 	GIN = GINL    ! Add turbulent viscosity if fluid boundary
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Tk Coefficient
!>    @brief     Compute the coefficients to solve the Turbulent Kinetic energy transport equation for each node in the domain.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] In Filling
...
!	 Calculo da gamma
	...
	GIET = flux(MuT(i,j)/Theta_k,MuT(i+1,j)/Theta_k,DXU(i),(DXP(i+1)/2.0D+0))

	GIWL = flux(Mu(i,j)         ,Mu(i-1,j)         ,DXU(i-1),(DXP(i-1)/2.0D+0))
	GIWT = flux(MuT(i,j)/Theta_k,MuT(i-1,j)/Theta_k,DXU(i-1),(DXP(i-1)/2.0D+0))

	GINL = flux(Mu(i,j),         Mu(i,j+1),         DYV(j),(DYP(j+1)/2.0D+0))
	GINT = flux(MuT(i,j)/Theta_k,MuT(i,j+1)/Theta_k,DYV(j),(DYP(j+1)/2.0D+0))

	GISL = flux(Mu(i,j),         Mu(i,j-1),         DYV(j-1),(DYP(j-1)/2D+0))
	GIST = flux(MuT(i,j)/Theta_k,MuT(i,j-1)/Theta_k,DYV(j-1),(DYP(j-1)/2.0D+0))
...
!	Coeficients				!
	...
	apo(i,j)=(RHO(i,j)*DXP(i)*DYP(j))/dt
     	...
     	ap(i,j)=ae(i,j)+aw(i,j)+an(i,j)+as(i,j)+apo(i,j) - Sp * DXP(i)*DYP(j)
      	...
      	b(i,j)=apo(i,j)*TKx(i,j) + Sc  * DXP(i)*DYP(j)
...
!	 Calculo da gamma
	...
	GIEL = flux(Mu(i,j),         Mu(i+1,j),         DXU(i),(DXP(i+1)/2.0D+0) )
	GIET = flux(MuT(i,j)/Theta_e,MuT(i+1,j)/Theta_e,DXU(i),(DXP(i+1)/2.0D+0))

	GIWL = flux(Mu(i,j)         ,Mu(i-1,j)         ,DXU(i-1),(DXP(i-1)/2.0D+0))
	GIWT = flux(MuT(i,j)/Theta_e,MuT(i-1,j)/Theta_e,DXU(i-1),(DXP(i-1)/2.0D+0))

	GINL = flux(Mu(i,j),         Mu(i,j+1),         DYV(j),(DYP(j+1)/2.0D+0))
	GINT = flux(MuT(i,j)/Theta_e,MuT(i,j+1)/Theta_e,DYV(j),(DYP(j+1)/2.0D+0))

	GISL = flux(Mu(i,j),         Mu(i,j-1),         DYV(j-1),(DYP(j-1)/2D+0))
	GIST = flux(MuT(i,j)/Theta_e,MuT(i,j-1)/Theta_e,DYV(j-1),(DYP(j-1)/2.0D+0))
...
!	Source terms				!
	...
	apo(i,j)=(RHO(i,j)*DXP(i)*DYP(j))/dt
     	...
     	ap(i,j)=ae(i,j)+aw(i,j)+an(i,j)+as(i,j)+apo(i,j)  - Sp * DXP(i)*DYP(j)
      	...
      	b(i,j)=apo(i,j)*e(i,j) + Sc  * DXP(i)*DYP(j)
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE U Coefficient
!>    @brief     Compute the coefficients to solve the momentum transport equation in the x-direction for each node in the domain.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!		MuT_j1 = flux(Mu(i,j+1),Mu(i+1,j+1),DXU(i),DXP(i+1)/2.0D+0)
!		MuT_p = flux(Mu(i,j),Mu(i+1,j),DXU(i),DXP(i+1)/2.0D+0)

!		MuT_n = flux(MuL_p,MuL_j1,DYV(j),DYP(j+1)/2.0D+0)

!		MuT_j_1 = flux(Mu(i,j-1),Mu(i+1,j-1),DXU(i),DXP(i+1)/2.0D+0)
!		MuT_s = flux(MuL_j_1,MuL_p,DYV(j-1),DYP(j)/2.0D+0)
...
!		if (j.eq.2) 		GIS= Mu(i,1) + Mu(i+1,1) ! If fluid boundary, add turbulent viscosity
!		if (j.eq.(Ny-1)) 	GIN= Mu(i,Ny) + Mu(i+1,Ny) ! If fluid boundary, add turbulent viscosity
...
!      			Ds=( GIS * DXU(i) ) / (Y(j) - Y(j-1))
      ...
      GIE=mu(i,j)+mut(I+1,J)
      GIW=mu(i,j)+mut(I,J)

!******
      D1P=FLUX(MuT(I,J),mut(I+1,J),DXU(I),DXP(I))
      D2N=FLUX(mut(I,J+1),mut(I+1,J+1),DXU(I),DXP(I))
            IF (J.EQ.NY-1) D2N=mu(i,j)                                             !"FRONTERA SOLIDA"
      D3S=FLUX(mut(I,J-1),mut(I+1,J-1),DXU(I),DXP(I))
            IF (J.EQ.2) D3S=mu(i,j)                                                !"FRONTERA SOLIDA"
!******
      F1=(DYP(J+1)/2.0D+00)/DYV(J)
      GIN=mu(i,j)+(PROPIEDAD(D1P,D2N,F1))
!***             IF (J.EQ.NY-1)GIN=mu+mut(I,J+1)                                 "FRONTERA FLUIDA"
             IF (J.EQ.NY-1)GIN=mu(i,j)                                             !"FRONTERA SOLIDA"
!******
      F1=(DYP(J)/2.0D+00)/DYV(J-1)
      GIS=mu(i,j)+(PROPIEDAD(D3S,D1P,F1))
!***             IF (J.EQ.2)GIS=mu+mut(I,J-1)                                    "FRONTERA FLUIDA"
             IF (J.EQ.2)GIS=mu(i,j)                                               ! "FRONTERA SOLIDA"
!******
!C
!CC    LOS FLUJOS FŽS SON APROXIMADOS POR EC. DE CONTINUIDAD
!C
           FE=FEP(I,J)+0.5D+00*(FNP(I+1,J-1)-FNP(I+1,J))
           DE=(GIE*DYP(J))/DXP(I+1)
           FW=FEP(I-1,J)+(0.5D+00*(FNP(I,J-1)-FNP(I,J)))
              IF (I.EQ.2)    FW=FEP(I,J)+(0.5D+00*(FNP(I,J)-FNP(I,J-1)))
              IF (I.EQ.NX-2) FW=FEP(I,J)+(0.5D+00*(FNP(I,J)-FNP(I,J-1)))
           DW=(GIW*DYP(J))/DXP(I)
           FN=0.5D+00*(FNP(I,J)+FNP(I+1,J))
             IF (J.EQ.NY-1) FN=0.0D+00                                        ! "FRONTERA SOLIDA"
           DN=(GIN*DXU(I))/DYV(J)
           FS=0.5D+00*(FNP(I,J-1)+FNP(I+1,J-1))
             IF (J.EQ.2)    FS=0.0D+00                                        ! "FRONTERA SOLIDA"
           DS=(GIS*DXU(I))/DYV(J-1)
...
!-----------------------------------------------------------------------------------------------!
			...
			apo=(flux(RHO(i,j),RHO(i+1,j),DXU(i),DXP(i+1)/2.0D+0)*DXU(i)*DYP(j)) /dt
     			...
     			apu(i,j)=ae(i,j)+aw(i,j)+an(i,j)+as(i,j)+apo
...
!C
!CC    CALCULO DE APU-NEIGHBOR PARA EL ALGORITMO SIMPLE Y SIMPLEC
!C
	DO 2125 I=1,NX-1
	DO 2125 J=1,NY
	...
	2125   CONTINUE
!C
!CC    FACTOR DE CORRECCIÓN DE VELOCIDAD U
!C
        DO 2300 J=2, NY-1
        DO 2300 I=2, NX-2
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE V Coefficient
!>    @brief     Compute the coefficients to solve the momentum transport equation in the y-direction for each node in the domain.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!		if (i .eq. Nx-1) GIE = flux( Mu(Nx-1,j),Mu(Nx-1,j+1),DYV(j),DYP(j+1)/2.0D+0 ) ! Add Mut if fluid boundary
!		if (i .eq. 2) GIW = flux( Mu(2,j),Mu(2,j+1),DYV(j),DYP(j+1)/2.0D+0 ) ! Add Mut if fluid boundary
...
!******
      D1P=FLUX(mut(I,J),mut(I,J+1),DYV(J),DYP(J))
      D2E=FLUX(mut(I+1,J),mut(I+1,J+1),DYV(J),DYP(J))
            IF (I.EQ.NX-1) D2E=mu(i,j)                                           !  "FRONTERA SOLIDA"
      D3W=FLUX(mut(I-1,J),mut(I-1,J+1),DYV(J),DYP(J))
            IF (I.EQ.2) D3W=mu(i,j)                                               ! "FRONTERA SOLIDA"
!******
      F1=(DXP(I+1)/2.0D+00)/DXU(I)
      GIE=mu(i,j)+(PROPIEDAD(D1P,D2E,F1))
!***             IF (I.EQ.NX-1)GIE=mu(i,j)+mut(I+1,J)                                 "FRONTERA FLUIDA"
             IF (I.EQ.NX-1)GIE=mu(i,j)                                             !"FRONTERA SOLIDA"
!******
      F1=(DXP(I)/2.0D+00)/DXU(I-1)
      GIW=mu(i,j)+(PROPIEDAD(D3W,D1P,F1))
!***             IF (I.EQ.2)GIW=mu(i,j)+mut(I-1,J)                                    "FRONTERA FLUIDA"
             IF (I.EQ.2)GIW=mu(i,j)                                                !"FRONTERA SOLIDA"
!******

      GIN=mu(i,j)+mut(I,J+1)
      GIS=mu(i,j)+mut(I,J)
!C
!CC    LOS FLUJOS FŽS SON APROXIMADOS POR EC. DE CONTINUIDAD
!C
           FE=0.5D+00*(FEP(I,J)+FEP(I,J+1))
             IF (I.EQ.NX-1) FE=0.0D+00                                         !"FRONTERA SOLIDA"
           DE=(GIE*DYV(J))/DXU(I)
           FW=0.5D+00*(FEP(I-1,J)+FEP(I-1,J+1))
             IF (I.EQ.2) FW=0.0D+00                                            !"FRONTERA SOLIDA"
           DW=(GIW*DYV(J))/DXU(I-1)
           FN=FNP(I,J)+0.5D+00*(FEP(I-1,J+1)-FEP(I,J+1))
           DN=(GIN*DXP(I))/DYP(J+1)
           FS=FNP(I,J-1)+(0.5D+00*(FEP(I-1,J)-FEP(I,J)))
              IF (J.EQ.2)    FS=FNP(I,J)+(0.5D+00*(FEP(I,J)-FEP(I-1,J)))
              IF (J.EQ.NY-2) FS=FNP(I,J)+(0.5D+00*(FEP(I,J)-FEP(I-1,J)))
           DS=(GIS*DXP(I))/DYP(J)
!C
!CC    CONTRIBUTION OF THE SOURCE TERM "SU"
!C
...
!-----------------------------------------------------------------------------------------------!
			...
			apv(i,j)=ae(i,j)+aw(i,j)+an(i,j)+as(i,j)+apo
...
!	end do
 ...
 2225   CONTINUE
!C
!CC    FACTOR DE CORRECCIÓN DE VELOCIDAD V
!C
        DO 2302 J=2, NY-2
        DO 2302 I=2, NX-1
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE P Coefficient
!>    @brief     Compute the coefficients to solve pressure correction equation for each node in the domain.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] In Filling
!!    @param     [out] Out Filling
...
Subroutine COEF_P(DXP,DYP,Ux,Vx,du,dv,ap,ae,aw,as,an,b,RES_P,Rmax_P,RHO,apu,apv,apunb,apvnb)
	...
	real(KIND = DP)  :: RES_P,De,Dw,Dn,Ds,resi,summ,Rmax_P,RHOe,RHOw,RHOs,RHOn
...
!			      +(RHO(i,j)-RHO(i,j) )*DXP(i)*DYP(j)/dt
      ...
      IF (I.EQ.NX-1) THEN
                          AE(I,J) = 0.0D+00                                     !"C.F.=LADO ESTE"
                     ELSE
                          DE=DYP(J)/(APU(I,J)-APUNB(I,J))
                          AE(I,J)=RHOe*DE*DYP(J)
      END IF

      IF (I.EQ.2)    THEN
                          AW(I,J) = 0.0D+00                                     !"C.F.=LADO OESTE"
                     ELSE
                          DW=DYP(J)/(APU(I-1,J)-APUNB(I-1,J))
                          AW(I,J)=RHOw*DW*DYP(J)
      END IF
!C
      IF (J.EQ.NY-1) THEN
                          AN(I,J) = 0.0D+00                                     !"C.F.=LADO NORTE"
                     ELSE
                          DN=DXP(I)/(APV(I,J)-APVNB(I,J))
                          AN(I,J)=RHOn*DN*DXP(I)
      END IF
!C
      IF (J.EQ.2)    THEN
                          AS(I,J) = 0.0D+00                                     !"C.F.=LADO SUR"
                     ELSE
                          DS=DXP(I)/(APV(I,J-1)-APVNB(I,J-1))
                          AS(I,J)=RHOs*DS*DXP(I)
      END IF
	...
	!b(i,j)=RHOw*DYP(j)*Ux(i-1,j) 		&
	 !     -RHOe*DYP(j)*Ux(i,j)  		&
	  !    +RHOs*DXP(i)*Vx(i,j-1)   		&
	   !   -RHOn*DXP(i)*Vx(i,j)	     	&
	    !  +(RHO(i,j)-RHO(i,j) )*DXP(i)*DYP(j)/dt	! Oju transitori

	b(i,j)=RHOo*(UX(I-1,J)-UX(I,J))*DYP(J)+&
            	RHOo*(VX(I,J-1)-VX(I,J))*DXP(I)
...
!-----------------------------------------------------------------------------------------------!
	...
	do j=2,Ny-1
		do i=2,Nx-1
			...
			if (b(i,j) .gt. Rmax_P) Rmax_P=DABS(b(i,j))
		end do
	end do
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Mass Flux
!>    @brief     Compute the mass flux crossing the control volume.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
subroutine Mass_Flux(U,V,RHO,Fep,Fnp)
	...
	Do j=1,Ny
		Do i=2,(Nx-2)
			Fep(i,j) = U(i,j)*( RHO(i,j)+RHO(i+1,j) ) / 2.0D+0
		end do
...
write(*,*) U(31,31),RHOo,RHOo*U(31,31),FEP(31,31)
stop
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     FUNCTION   Average Flux
!>    @brief     Compute the value of a property in a boundary assuming linear variation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!>    @brief     Compute the value of a property in a boundary assuming through the harmonic mean.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!>    @brief     Update all the therm-properties based on the temperature field.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Correct
!>    @brief     Correct U,V and P matrixes as a results of the SIMPLE algorithm.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!--------------------------------------------------------------------------------------------------
!     FUNCTION   A(n,Pe)
!>    @brief     Choose the numerical scheme of the convective terms.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE LBL X
!>    @brief     Solver line by line in x.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Dimensionless LBL
!>    @brief     Solver line by line alternative directions
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] In Filling
!!    @param     [out] Out Filling
...
!C
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Mass Fluxes
!>    @brief     Compute mass fluxes crossing the control volume boundaries.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!>    @brief     Compute the value of a property in the CV boundary assuming linear variation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.

...
!!    @brief   Module containing the specification of each specific model for the turbulence.
!!    @authors Jan Mateu
!!    @date    22/08/2017
!--------------------------------------------------------------------------------------------------
module rans_models
...
!--------------------------------------------------------------------------------------------------
!     FUNCTION   MuT Model
!>    @brief     Compute the turbulent viscosity on each point in the domain from the value of k-e or k-w.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!			f_mu = 0.025D+00 +( 1.0D+0-EXP(-(arg)**(0.75D+0)) ) &
!	 		    *( 0.975D+00 + (0.001D+0/Re_t)*EXP(-(arg2*arg2) ) )
...
!if ( (iter==2).and.(i==3).and.(j==10)) then
!	write(*,*)
!	write(*,*) Cmu,f_mu,RHO(i,j),TK(i,j),e(i,j)
!	write(*,*)
!end if
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE RANS Inital constants
!>    @brief     Initialize the constant for each specific turbulent model.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE RANS Tk Source Terms
!>    @brief     Compute the source terms of the turbulent kinetic energy transport equation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE RANS e Source Terms
!>    @brief     Compute the source terms of the turbulent kinetic energy dissipation transport equation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
subroutine rans_source_terms_e(i,j,u,v,Mut,mu,T,rho,e,TK,sc,sp)
	...
	SELECT CASE (rans_model)
		...
		CASE (2:3) ! k-epsilon JL and LS
			...
			Pk(i,j) = C1e*abs(f1)*Pk(i,j)*e(i,j)     / (TK(i,j))
			Gk(i,j) = C1e*C3e*Gk(i,j) *e(i,j)        / (Tk(i,j))
			Dis = C2e*abs(f2)*RHO(i,j)*e(i,j)*e(i,j) / (TK(i,j))
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE RANS Boundary
!>    @brief     Specify the value of the turbulent kinetic energy dissipation on the boundaries depending on the model.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!>    @brief     Numerical first order derivative operation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.
...
!-----------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------
!     SUBROUTINE Second Order Derivative
!>    @brief     Numerical second order derivative operation.
!!    @authors   Jan Mateu
!!    @date      22/08/2017
!!    @param     [in] Filling.
!!    @param     [out] Filling.

...
subroutine dependent_var(DeltaT,Hx,Hy,dt,Tref,CONTERo,Po,Muo)
	...
	SELECT CASE (case_jet)
		CASE (1)
			Po 				= 1d+0 * 101325.0D+00 
			h 				= 0.05d+00
			T_jet 			= 610d+0
			T_coflow 		= 610d+0	
			Reynolds		= 3200d+0
		...
		CASE (3)
			...
			ratio_conter	= 1d+0							
		CASE DEFAULT
			stop "jet case unknown"
	END SELECT

	Tref=(T_jet+T_coflow)/2.0D+00
	DeltaT=T_jet-T_coflow
!T_jet=Tref

	RHOo=Po/(R*T_jet);
	Cpo = 2157.0d0

	contero=  (+5.3644d-8 * T_jet * T_jet   & 
              +3.9243d-5 * T_jet          &
              +5.0855d-3)*ratio_conter

	Muo= (-4.6763d-12  * T_jet * T_jet   & 
         +4.8259d-8   * T_jet          &
         -5.9340d-6)*ratio_mu

	u_jet = ( Reynolds * Muo / (h * RHOo) ) / 0.90D+0
	u_coflow = u_jet




	write(*,*) "__case jet =",case_jet,"___________"
	write(*,*) "Pressao = ", Po
	write(*,*) "Muo = ", Muo
	write(*,*) "RHOo = ", RHOo		
	write(*,*) "contero = " , contero
	write(*,*) "u_jet = ", u_jet
	write(*,*) "u_coflow = ", u_coflow
	write(*,*) "Re = ", Reynolds
	write(*,*) "rho u = ", rhoo * u_jet
	write(*,*) "DeltaT = ", DeltaT	
	write(*,*) "_____________"
	...
	do j=1,Ny
 		...
 		U_in(j) = ((u_jet+u_coflow)/2d+0    + (u_jet-u_coflow)/2d+0    * &
                         dtanh( (-abs(y(j)) + (h/2D+0)) / (2D+0*theta) ) )

 		T_in(j) = ((T_jet+T_coflow)/2d+0    + (T_jet-T_coflow)/2d+0    * &
                         dtanh( (-abs(y(j)) + (h/2D+0)) / (2D+0*theta) ) )

		TK_in(j) = 0.07D+0*0.07D+0 * U_in(j) * U_in(j)
	...
	SELECT CASE (rans_model)
		...
		e_in(j) = Cmu * TK_in(j)**(1.5D+0)  / (0.3D+0 * (h/2.0D+0)) !(0.01d+0*3.0D+0/4.0D+0) !

		! Aumentando a dissipaçao na entrada o jato demora muito mais para se desenvolver

		CASE (4:5) ! k-w 

		e_in(j) = Cmu * TK_in(j)**(0.5D+0) / (3.0D+0 * (Hy/2.0D+0))
...
subroutine start (U,V,P,T,Tref,rho,Fep,Fnp,TK,e)	
	...
	SELECT CASE (case_jet)
		CASE (1)
			case_jet_string="iso"
		CASE (2)
			case_jet_string="Re1500"
		CASE (3)
			case_jet_string="Re500"					
		CASE DEFAULT
			stop "jet case unknown"
	END SELECT
...
open(unit=99, file ='./outputdata/matlab/ite_coupling' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "first ite_coupling"
	ite_coupling = 1
else
	read(99,*) ite_coupling
	ite_coupling = ite_coupling + 1
	write(*,*) "%%%%%%%%%%%%%%         ite_coupling         %%%%%%%%%%%%%%%%%%%%%%"
	write(*,*) "%%%%%%%%%%%%%%       ",ite_coupling,"       %%%%%%%%%%%%%%%%%%%%%%"
end if
close(99)
...
open(unit=56, file ='./outputdata/matlab/u_re' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "u-velocity file not found, u=u(y) everywhere"
	Do j=1,Ny
			U(:,j)=U_in(j)
	end do
else
...
open(unit=67, file ='./outputdata/matlab/v_re' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')

if (stat .ne. 0) then
    write(*,*) "V-velocity file not found, v=1e-20 everywhere"
	V(:,:) = 0.0D+0
	Do j=2,(Ny-2)
		Do i=2,Nx-1
			V(i,j)=1.0D-20
...
open(unit=3, file ='./outputdata/matlab/T' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "Temperature file not found, T=Tref everywhere"
	Do j=1,Ny
		Do i=1,Nx
			T(i,j)=Tref
		end do
...
open(unit=88, file ='./outputdata/matlab/pressure' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "Pressure file not found, P=0 everywhere"
	Do j=1,Ny
		Do i=1,Nx
			P(i,j)= 0.0D+0
		end do
...
open(unit=18, file ='./outputdata/matlab/rho' //trim(case_jet_string)// '.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')

if (stat .ne. 0) then
    write(*,*) "Rho file not found, P=RHOo everywhere"
	Do j=1,Ny
		Do i=1,Nx
			RHO(i,j)=RHOo 
		end do
...
!********** Read TKE ***********!
open(unit=7, file ='./outputdata/matlab/TK.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "TKE file not found, TK=0.5 everywhere"
	Do j=1,Ny
		Do i=1,Nx
			TK(i,j)=TK_in(j)*0.5D+0
		end do
	end do
else
	do i = 1,Nx
...
!********** Read E ***********!
open(unit=8, file ='./outputdata/matlab/e.dat',ACTION='read',IOSTAT=stat,STATUS='OLD')
if (stat .ne. 0) then
    write(*,*) "epsilon file not found, e=0.5 everywhere"
	Do j=1,Ny
		Do i=1,Nx
			e(i,j)=e_in(j)*0.5D+0
		end do
...
!********** Read Prad ***********!
OPEN(UNIT=31, FILE="inputdata/P_rad.txt",ACTION='read',IOSTAT=stat,STATUS='OLD')


if (stat .ne. 0) then

    write(*,*) "P_rad.txt not found, Prad=0 everywhere"
    P_rad = 0
else
	do i = 1,Nx
	    read(31,*) P_rad(i,:)
...
!   	Pm = ( Px(i,Ny-1) + Px(i,Nx-2) ) / 2.0D+0
!   	Pw = Px(i,Ny-1) + ( Px(i,Ny-1) - Px(i,Nx-2) ) * ( y(Ny) - y(Ny-1) ) / ( y(Ny-1) - y(Ny-2) )

! 		vv_in = sqrt( abs( RHOo*Vx(i,Ny-2)**2.0D+0 / 2.0D+0 + (Pm-Pw)) * 2.0D+0 / RHOo ) 

!   	if (Pw.gt.Pm) outflow=-1.0D+0 ! INFLOW at the north boundary
!   	if (Pw.eq.Pm) outflow= 0.0D+0 ! no cross-flow at the north boundary
!   	if (Pw.lt.Pm) outflow=+0.0D+0 ! outflow at the north boundary

!   	b(i,Ny-1)	=	 outflow * min(vv_in,u_coflow/1.0D+0)

!  	if (i.le.4) b(i,Ny-1)=0.0D+0 
!  	if (i.ge.Nx-5) b(i,Ny-1)=0.0D+0 
...
!**************************************************************************
!*******            RESUELVE EL SIST. DE ECS. ORDINARIAS            *******        
!*****   (METHOD: LINE GAUSS SEIDEL ALTERNATING DIRECTION IMPLICIT)   *****        
!**************************************************************************
!C        
!***************   SOLUCIÓN DE LA MATRIZ TRIDIAGONAL   ********************
!**********  LA MATRIZ ES RESUELTA USANDO EL ALGORITMO DE THOMAS   ********
!C
      SUBROUTINE LGS_ADI(I0,J0,I1,J1,AP,AE,AW,AN,AS,B,FHI,NPAS,RELAX)                 
!C        
...
!CC    ****  1er.Punto en "X"  ****
!C
                    BETA=B(I0,J)+AS(I0,J)*FHI(I0,J-1)+ AN(I0,J)*FHI(I0,J+1)
...
!C
      DO 334 I=I0+1,I2
                       BETA=B(I,J)+AS(I,J)*FHI(I,J-1)+ AN(I,J)*FHI(I,J+1)
...
!CC    ****  Ultimo Punto en "X"  ****
!C
                    BETA=B(I1,J)+AS(I1,J)*FHI(I1,J-1)+ AN(I1,J)*FHI(I1,J+1)

      IF(J.EQ.J0)   BETA=B(I,J1)+AN(I1,J)*FHI(I1,J+1)   
      IF(J.EQ.J1)   BETA=B(I,J1)+AS(I1,J)*FHI(I1,J-1)  
...
!CC    ****  1er.Punto en "Y"  ****
!C
                    BETA=B(I,J0)+AW(I,J0)*FHI(I-1,J0)+ AE(I,J0)*FHI(I+1,J0)
...
!C
      DO 324 J=J0+1,J2
                       BETA=B(I,J)+AW(I,J)*FHI(I-1,J)+AE(I,J)*FHI(I+1,J)  
...
!CC    ****  Ultimo Punto en "Y"  ****
!C
                    BETA=B(I,J1)+AW(I,J1)*FHI(I-1,J1)+AE(I,J1)*FHI(I+1,J1)  
